...
# ╔═╡ e742c248-b17d-4310-99c3-8cc526a069c9
begin
	using Pluto
	using Plots	
...
# ╔═╡ febba034-9a96-485a-b8fa-0deb78577c6d
md"
---
##### 1. Topics
...
# ╔═╡ 95d17a86-9322-47b7-8ce6-638bbb5ce2ff
md"
---
##### 2. Libraries
"

# ╔═╡ 04c8ef1a-c7e6-4f87-b55a-13a283aa54be
md"
---
...
md"
##### 3.1. *Style Variants* of the Linear Recursive *Factorial* Function
"
...
md"
---
##### 3.2. Approximating the *true* function *FAC* by *Non*recursive *One*-steps
###### 3.2.1 Transpilation of Pearce's proposals  from Scheme to Julia
...
function multiStep(n, oneStep, initialApprox)
	zerop(n) ? 
		initialApprox : 
		oneStep(multiStep(pred(n), oneStep, initialApprox))
...
md"
---
###### 3.2.2 Proof: $fact_{iterate}(n) =  FACT = n\;!$ for $0 \le n \le 1000$
(Pearce, 1998, p.168)
...
let
	...
	all([fact(i) == prod(1:i) ? 
		true : 
		false for i = 1:nmax]) ? 
			println("fact == to FACT == n! upto $nmax. So it's a fixed point (at least for this range) --> :) ") : 
			println(" discrepancy ! --> :(")
end # let
...
md"
---
##### 3.3 Definition of Self-improving (*oneStep*) Improvers 
(Pearce, 1998, p.168f)
...
md"
---
We made this *substitution* in Pearce's code (Pearce, 1998, p.168f) to achieve similarity with $Y_{applicative}$:
...
md"
---
###### Pearce's Fixed Point Operator $fix$ 
...
md"
---
##### 4. The Fixed-point Operator Y ($\lambda$-calculus inspired)
###### 4.1 A simple *Iteration Pattern (IP)* of the $\lambda$-Calculus
...
md"
---
###### 4.2 Three steps of *one* *IP*'s evaluation cycle
...
md"
---
###### 4.3 The *Normal-order* fixed-Point Combinator $Y$
...
$\Lambda = \text{ set of } \lambda \text{-terms }$ 

$\;$

(Barendregt & Barendson, 2020, p.9\)

$\;$

*and that $FX = X$ is provable in the $\lambda$-calculus*

$\mathbf{\lambda} \vdash FX = X$ 

$\;$

(Barendregt & Barendson, 2020, p.11\).
...
md"""
---
###### 4.4 The *Applicative-order* Fixed-Point Combinator $Y$
...
md"
---
###### 4.5 The *Applicative-order* fixed-Point Combinator $Y$ in Julia code
...
md"
---
###### 4.6 Trace of $Y$'s application by Just-in-Time (*JIT*)-Substitutions
...
$(x \rightarrow (fac \rightarrow (n \rightarrow zerop(n)\; ? 1 :$

$\;$

$n * fac(pred(n))))(y \rightarrow x(x)(y)))$
...
md"
---
##### 5. Examples: Applications of the *fix-point combinator* $Y1$
###### 5.1 to simple nonrecursive $cos$-function
...
md"
---
###### 5.2 to the simple *non*recursive $f := x = x^2 -2$
In princi
...
let #------------------------------------------------------------------------------- 
    # we are looking for the fixed points of h
	f = n -> n*n-2
...
md"
---
###### 5.3 to the recursive computation of $sqrt$
"
...
md"
---
###### 5.4 to [Heron's sqrt-algorithm](https://en.wikipedia.org/wiki/Methods_of_computing_square_roots#Heron's_method) - derived from [Newton's method](https://en.wikipedia.org/wiki/Newton%27s_method).
...
$x_{n+1}= x_n - \frac{f(x_n)}{f'(x_n)}\left[f := (x^2 - a); f' := 2x\right]$
...
# ╔═╡ 50b9b182-40a1-4851-8648-d1a518d51b0d
md"
---
##### 6. Summary
