...
# ╔═╡ a7ba2f90-69e5-4440-92b1-fd3341b34a1c
begin 
	#----------------------------------------------------------------------------
	using Pluto
	using Plots	
	...
	println("pkgversion(IntervalArithmetic) = ", pkgversion(IntervalArithmetic))
	#----------------------------------------------------------------------------
end # begin
...
md"
=====================================================================================
##### SICP: [2.1.4 Exercise Interval Arithmetic](https://mitp-content-server.mit.edu/books/content/sectbyfn/books_pres_0/6515/sicp.zip/full-text/book/book-Z-H-14.html#%_sec_2.1.4)

###### file: PCM20210915\_SICP\_2.1.4\_Exercise\_Interval\_Arithmetic.jl

###### code: Julia/Pluto.jl-code (1.11.5/0.20.06) by PCM *** 2025/05/04 ***

=====================================================================================

"

# ╔═╡ 6652bd43-d271-48c3-b9ed-030dbc8b21da
md"
---
##### 0. Introduction

*Interval arithmetic (also known as interval mathematics; interval analysis or interval computation) is a mathematical technique used to mitigate rounding and measurement errors in mathematical computation by computing function bounds. Numerical methods involving interval arithmetic can guarantee relatively reliable and mathematically correct results. Instead of representing a value as a single number, interval arithmetic or interval mathematics represents each value as a range of possibilities.*(Wikipedia) 

"

# ╔═╡ bd8e36cf-c98d-4021-b692-fcfa1a418873
md"
---
##### 1. Topics
...
# ╔═╡ b1561073-6fc5-47d1-98dd-0af3e68dec8b
md"
---
...
md"
---
##### 3.1 [Intervals](https://en.wikipedia.org/wiki/Interval_arithmetic) ...
... are quantities like

$[x] \equiv [a, b] = \{x \in \mathbb R \;|\; a \ge x \ge b\}\;;\;\text{ where } a, b \in \mathbb R$
...
md"
##### 3.2 Level 2: Abstract [Operations](https://en.wikipedia.org/wiki/Interval_arithmetic)
"
...
---
    -------------------------------------------------------------------------------
     Abstract                                                              level 2
     Operators     addInterval  subInterval  mulInterval  divInterval      Domain
    ------------------------------------------------------------------------------ 
     Constructor /                      makeInterval                       level 1  
     Selectors              lowerBound            upperBound  
    -------------------------------------------------------------------------------
...
$[x] / [y] := [x_1, x_2] \times \frac{1}{[y_1,y_2]} = [x_1, x_2] \times \left[\frac{1}{y_2}, \frac{1}{y_1}\right]$
...
---
##### 3.3 Level 1: 
###### 3.3.1 Constructors $makeInterval$ and $cons$
---
###### makeInterval:
$makeInterval: \mathbb R \times \mathbb R \rightarrow [\mathbb R \times \mathbb R]$
...
md"
---
###### 3.3.2 Selectors $lowerBound, upperBound, car$ and $cdr$
...
---
##### 4.1 [Electricity Laws](https://en.wikipedia.org/wiki/Electrical_resistance_and_conductance)
...
# ╔═╡ 93551bc8-6964-4ab0-bbde-6b35982a826b
md"
###### *Resistor1* with 6.8 Ohm and 10% Tolerance
"

# ╔═╡ 97791387-5729-4db9-9c78-472254505f89
# resistor1 = make_interval(-(6.8, *(6.8, 0.1)), +(6.8, *(6.8, 0.1))) # prefix

# ╔═╡ f3d90221-4487-487b-ad62-5fe8b461f76a
resistor1 = makeInterval(6.8 - 6.8 * 0.10, 6.8 + 6.8 * 0.10)

# ╔═╡ 11721f93-3c67-4dba-9db8-91aa7a076570
md"
###### *Resistor2* with 4.7 Ohm and 5% Tolerance
"

# ╔═╡ 64a475b6-fec7-446a-a622-896a6ee7812b
# resistor2 = make_interval(-(4.7, *(4.7, 0.05)), +(4.7, *(4.7, 0.05))) # prefix

# ╔═╡ 3bc0243b-c071-40c6-b8fd-d4420d093bc6
resistor2 = makeInterval(4.7 - 4.7 * 0.05, 4.7 + 4.7 * 0.05)

# ╔═╡ 0d5db774-8136-472f-b0a3-a3197ce360b2
md"
###### $one$ is a precise $1$
"
...
# ╔═╡ 8b2772df-ba5d-4a87-8ab9-869b00ee8cc8
md"
###### *Conductor1*
"
...
conductor1 = divInterval(one, resistor1) # 1/R_1

# ╔═╡ d5de080a-069a-44e3-b216-8d82ee034741
md"
###### *Conductor2*
"
...
# conductance = 1/resistance
conductor2 = divInterval(one, resistor2) # 1/R_1

# ╔═╡ 4da683cc-758d-4fad-bf6c-df4ec9e146de
md"
---
###### Resistance $R_p$ of two Parallel Resistors $R_1$ and $R_2$
"
...
**Fig. 2.1.4.1**: Resistance of two parallel resistors
...
# ╔═╡ 2b970ad3-9fe8-4e00-bf27-54e4211f1e8d
md"
###### Conductance of two Parallel Conductors

$\frac{1}{R_p} = \frac{1}{R_1} + \frac{1}{R_2} = \frac{R_2}{R_1 \cdot R_2} + \frac{R_1}{R_1 \cdot R_2} = \frac{R_1 + R_2}{R_1 \cdot R_2} = \frac{1}{R_1R_2/(R_1+R_2)}.$
...
conductorP = addInterval(conductor1, conductor2)

# ╔═╡ fdb29214-036d-4ad3-9369-e6690eaa2098
md"
...
resistorP = divInterval(one, conductorP) # R_p = 1/(1/R_1 + 1/R_2)
...
md"
###### 4.1 Interval arithmetic with Julia package [*IntervalArithmetic.jl*](https://github.com/JuliaIntervals/IntervalArithmetic.jl)
"

# ╔═╡ 45da67c4-db32-404e-a1ea-c6b198099235
resistor1IntAr = interval(6.8 - 6.8 * 0.10, 6.8 + 6.8 * 0.10)

# ╔═╡ fab320b0-5198-48bf-91d7-53e345fabfbb
resistor2IntAr = interval(4.7 - 4.7 * 0.05, 4.7 + 4.7 * 0.05)

# ╔═╡ dfbe63b2-174f-4c43-a930-d3857e619019
oneIntAr = interval(1.0, 1.0)

# ╔═╡ f6df3322-977c-4436-9e7d-551973db36af
# conductance = 1/resistance
conductor1IntAr = oneIntAr / resistor1IntAr  # 1/R_1

# ╔═╡ 5b8d6e8b-9d67-4933-a3c7-76dec4fbc4e5
conductor2IntAr = oneIntAr / resistor2IntAr  # 1/R_2

# ╔═╡ c9fcb984-397c-4c76-9ba4-c25ffdb280e5
conductorPIntAr = conductor1IntAr + conductor2IntAr

# ╔═╡ b4730d19-512c-43ca-bf71-3de6c873c771
# resistance = 1/conductance
resistorPIntAr = oneIntAr / conductorPIntAr # R_p = 1/(1/R_1 + 1/R_2)

# ╔═╡ 5ed9d7ab-74a0-48c2-b78c-959329e63eb9
md"
---
##### 5. Summary
We demonstrated how to obtain the *tolerance* interval of two parallel resistors, when their single tolerance is known. One implementation uses the transpilation of SICP-Scheme the other the Julia package $IntervalArithmetic$.
"
...
---
##### 6. References
- **Abelson, H., Sussman, G.J. & Sussman, J.**; [*Structure and Interpretation of Computer Programs*](https://mitp-content-server.mit.edu/books/content/sectbyfn/books_pres_0/6515/sicp.zip/full-text/book/book.html), Cambridge, Mass.: MIT Press, (2/e), 1996 last visit 2025/05/04
- **Abelson, H., Sussman, G.J. & Sussman, J.**; [*Structure and Interpretation of Computer Programs*](https://web.mit.edu/6.001/6.037/sicp.pdf), Cambridge, Mass.: MIT Press, (2/e), 2016 last visit 2025/05/04
...
- **Wikipedia**; [*Interval arithmetic*](https://en.wikipedia.org/wiki/Interval_arithmetic); last visit: 2025/05/04
...
manifest_format = "2.0"
project_hash = "2cc4ad2f7b58d33214502ad52f3573e60079568a"
