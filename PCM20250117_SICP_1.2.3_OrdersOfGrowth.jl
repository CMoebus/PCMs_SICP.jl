...
# ╔═╡ d946eeb9-55b5-4a3c-ac40-bf7ddac67ee5
begin 
	using Pluto
	using Plots	
	using LaTeXStrings
...
# ╔═╡ b4111840-d4d0-11ef-0866-b17759c214fd
md"
==================================================================================
...
##### Julia/Pluto.jl-code (1.12.5/0.20.4) by PCM *** 2026/03/18 ***

==================================================================================
"

# ╔═╡ 468f9ddd-d66d-4e7b-ad96-1a15c01f62bd
md"
##### 0. Introduction
...
---
##### 1. Topics
- *order of growth*, *Ordnung*
- *analysis of algorithms*
- *Bachmann-Landau notation*
"

# ╔═╡ cfdd4878-39a8-4d3e-a96d-39994e24aba9
md"
---
##### 2. Libraries
"
...
md"
---
###### 3.1 Measure of Growth in Time and Space (= *Complexity*): *big-O, big-Ω, big-Θ-notation*

Rates of growth in time and space (= *complexity*) can be described by the *big-O, big-Ω, big-Θ-notation* (Skiena, 2012, p.35). *SICP* stresses the importance of $\Theta(g(n))$:

- *upper* bound: $f(n) = O(g(n))$ means $c \cdot g(n)$ is an *upper* bound on $f(n)$ for all $n_0 \le n$:
$\exists c, n_0 \forall n \ge n_0: f(n) \le cg(n)$

- *lower* bound: $f(n) = Ω(g(n))$ means $c \cdot g(n)$ is an *lower* bound on $f(n)$ for all $n_0 \le n$:
$\exists c, n_0 \forall n \ge n_0: f(n) \ge cg(n)$

- *tight* bound: $f(n) = \Theta(g(n))$ means $c_1 \cdot g(n)$ is an *upper* bound and $c_2 \cdot g(n)$ is an *lower* bound on $f(n)$ for all $n_0 \le n$:
$\exists c_1, c_2, n_0 \forall n \ge n_0: c_2g(n) ≤ f(n) \le c_1g(n)\;\square$

"

# ╔═╡ 0a899972-cd86-48c7-9890-50c30a4505b2
function plotONotations()
	size    = (900, 400)
	xlimits = (0, 10)
	ylimits = (-5, 120)
	#----------------------------------------------------------------------
	...
	#----------------------------------------------------------------------------
	# Big-O
	p1 = plot(ns, f.(ns), label=L"f(n)", linewidth=2, xlimits=xlimits, ylimits=ylimits)
	plot!(p1, ns, c1 .* g.(ns), label=L"c_1*g(n)", linestyle=:dash, color=:red)
	vline!(p1, [2.5], label=L"n_0", color=:magenta)
	title!(p1, L"Big-O")
	#-----------------------------------------------------------------------------
	# Big-Ω
	p2 = plot(ns, f.(ns), label=L"f(n)", linewidth=2, xlimits=xlimits, ylimits=ylimits)
	plot!(p2, ns, c2 .* g.(ns), label=L"c_2*g(n)", linestyle=:dash, color=:green)
	vline!(p2, [2.5], label=L"n_0", color=:magenta)
	title!(p2, L"Big-Ω")
	#------------------------------------------------------------------------------
	# Big-Θ
	p3 = plot(ns, f.(ns), label=L"f(n)", linewidth=2, xlimits=xlimits, ylimits=ylimits)
	plot!(p3, ns, c1 .* g.(ns), label=L"c_1*g(n)", linestyle=:dash, color=:red)
	plot!(p3, ns, c2 .* g.(ns), label=L"c_2*g(n)", linestyle=:dash, color=:green)
	vline!(p3, [2.5], label=L"n_0", color=:magenta)
	title!(p3, L"Big-Θ")
	#------------------------------------------------------------------------------
	plot(p1, p2, p3, layout=(1,3), size=size)
end # function plotONotations

# ╔═╡ 7b3ab30f-56a7-4a04-8034-317608670283
plotONotations()

# ╔═╡ 3de363bd-8984-46c1-a96d-8ef3c5a2107b
md"
---
**Fig. 1.2.3.1**: Graphs of big-Measures (cf.: Fig. 3.2, Cormen et al., 2022, 4/e, ch.03, p.54; Fig.2.3, Skiena, 2012, p.35))
"

# ╔═╡ 14ef0a40-d487-451c-8588-6ca4ea7daebc
md"
---
##### 3.2 Survey of Some Common *Complexities*

Here we display *worst* *big*-$O$ orders wheras SICP presents *approximate*  *big*-$\Theta$ orders.

$\begin{array}{c|c|c|c}
\mbox{complexity}  & O(g(n)), \Theta(g(n)) & \mbox{function} & \mbox{description}\\
\hline  
\mbox{constant}    & O(1), \Theta(1) & fac(n) & \mbox{lin.recursive (space(tco); iterative) } \\
\mbox{}            &           & fib(n)   & \mbox{nonrecursive (space (tco)}           \\
\mbox{}            &           & gcd(n, m)) & \mbox{(non)recursive (space) }       \\
\hline
\mbox{logarithm. (n > m)} & O(log\ n) & gcd(n,m) & \mbox{lin.recursive (time); iterative}        \\
\hline
\mbox{linear}  & O(n), \Theta(n) & fac(n) & \mbox{lin. recursive (space (no tco), time) }           \\
...
\mbox{}            &           &          & \mbox{with } ψ = 1.618033988749895     \\
\hline 
\end{array}$
"
...
function plotOfComplexities()
	let ns = [n for n in 1:0.1:10]
		...
		plot!(n -> n^2, ns, label=L"O(n^2)")          # O(n^2)
		plot!(n -> n^3, ns, label=L"O(n^3)")          # O(n^3)
		plot!(n -> ψ^n, ns, label=L"O(ψ^n)")          # O(ψ^n)
		plot!(n -> 2^n, ns, label=L"O(2^n)")          # O(2^n)
	end # let
end # function plotOfComplexities
...
md"
**Fig. 1.2.3.2**: *Graphs of functions commonly used in the analysis of algorithms*
"
...
- **Cormen, Th.H., Leiserson, Ch.E., Rivest, R.L., & Stein, C.**; *Introduction to Algorithms*, Cambridge, Massachussetts: MIT Press, 2022, 4/e 

- **Skiena, S.S.**; *The Algorithm Design Manual*, Cham, Switzerland: Springer, 2012, 3/e

- **Wikipedia**; [*The Analysis of Algorithms*](https://en.wikipedia.org/wiki/Analysis_of_algorithms); last visit 2026/03/18

- **Wikipedia**; [*Bachmann*](https://en.wikipedia.org/wiki/Paul_Gustav_Heinrich_Bachmann); last visit 2025/01/18

- **Wikipedia**; [*Big-O-Notation*](https://en.wikipedia.org/wiki/Big_O_notation); last visit 2025/01/17

- **Wikipedia**; [*Landau*](https://en.wikipedia.org/wiki/Edmund_Landau); last visit 2025/01/18
