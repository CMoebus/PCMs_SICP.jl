...
using Statistics, Plots, LaTeXStrings, LinearAlgebra, GLM
...
##### Julia/Pluto.jl-code (1.9.3/19.27) by PCM *** 2023/09/29 ***
...
function dataModelComparison(x, y; xlabel="Model", ylabel="Ratings", title="Data-Model-Comp.")
...
md"
---
##### 1.3.1.3.1 Regression Models

Thurstone's Prediction of Choice ([Thurstone, 1945](https://link.springer.com/article/10.1007/BF02288891); [Ahrens & Möbus, 1965](http://oops.uni-oldenburg.de/2729/1/PCM1968.pdf)) and our [Voting model](https://uol.de/f/2/dept/informatik/ag/lks/download/Probabilistic_Programming/JULIA/Pluto.jl/SICP/PCM20210811_SICP_1.3.1.1_Procedures_as_Arguments_I.html?v=1694519967) are cognitive psychological models not mentioned in recent cognitive decision textbooks like (Wang & Busemeyer, 2021; Katsikopoulos, 2023). This is unfortunate because they are very practical, using easy-to-survey attitude measures with easy-to-use rating scales.

Here demonstrate a third *statistical* alternative not mentioned in both books, too. It is the use of standard OLS-Regression. For the computations we need no GLM-package. It suffices to use Julia's *matrix division operator* $\text{\\}.$ 
We demonstrate that the knowledge of only one or two categories in the *middle* of a rating scale is sufficient to predict precisely the percentage or probability of 1st choices. 
"

# ╔═╡ 390b1aad-a075-45b9-be77-ba9c0b460dab
md"
---
###### Thurstone's 1st Dataset
"
...
# predictions from Thurstone's 1st choice model
y1ThurstoneModel = [0.475799, 0.301452, 0.230428] 
...
'Matrix division using a polyalgorithm. For input matrices $\mathbf A$ and $\mathbf B$, the result $\mathbf X$ is such that $\mathbf{A*X == B}$ when $\mathbf A$ is square. The solver that is used depends upon the structure of $\mathbf A$. If $\mathbf A$ is upper or lower triangular (or diagonal), no factorization of $\mathbf A$ is required and the system is solved with either forward or backward substitution. For non-triangular square matrices, an LU factorization is used.For rectangular $\mathbf A$ the result is the *minimum-norm least squares* solution computed by a pivoted *QR* factorization of $\mathbf A$ and a rank estimate of $\mathbf A$ based on the $R$ factor. so that:' 

$\mathbf{A \cdot X == B}.$

In our problem of predicting *1st choice* votings:

$\mathbf{X' \cdot \beta == y}.$

$\mathbf{ A := X', X := \beta, B := y}$

where:

$\mathbf {X_{nCats, nS}} \text{ = matrix of attitude ratings}$
$\mathbf {X'_{nS, nCats}} \text{ = transposed matrix of attitude ratings}$
$\mathbf {\beta_{nCats, 1}} \text{ = column vector of regression estimates (= weights of categories)}$
$\mathbf {y_{nS, 1}} \text{ = column vector of voting data }$
$nCats \text{ = number of rating categories }$ 
$nS \text{ = number of stimuli }$ 
...
md"
First we solve the regression problem with a model with maximal number of parameters: for each of the $nCats$ categories *one* parameter. This results in *overfitting*. The prognoses are *exact*. Then, in a second step we reduce the number of parameters  to a number less than $nCats$. The selection of predictors (categories) is guided by their positive and negative contribution to overall importance.
"

# ╔═╡ daa9bbf3-fb5c-4870-bb7a-8e07f8b07593
function firstChoiceRegression(X, y)
	β = \(X, y)
	yHat = X*β
	sumYHat = sum(yHat)
	e = y - yHat
	eSSQ = e'e
	r = cor(y, yHat)
	β, yHat, sum(yHat), eSSQ, r
end # function firstChoiceRegression

# ╔═╡ b0ce4e86-5737-4144-aef5-dc988d756b16
y1ThurstoneModel

# ╔═╡ 3ff052e0-fd05-41d4-88e3-e840bef6d27a
firstChoiceRegression(y1', y1ThurstoneModel)
...
md"
###### OLS-Regression with Single Most Important Regressor
Extract 1 column of y1 with smallest error sum-of-squares $eSSQ$. This generates good predictions: 

- for category 2: $eSSQ = 0.0351$ and *positive* single $r(y, yHat)=0.8487$ 

The order of regression predictions is the same as the order of Thurstone Model predictions:
...
function lookForSingleBestCategory(X, y)
	nS, nCats = size(X)
	xi = zeros(Float64, nS)
	eSSQ_Min = 1.0; indexMin = 0; rMin = 0.0
	for i in 1:nCats
		for j in 1:nS
		 	xi[j] = X[j, i] # copy column i out of X
		end # for j
		β, yHat, sum(yHat), eSSQ, r = firstChoiceRegression(xi, y)
		if eSSQ < eSSQ_Min
			eSSQ_Min = eSSQ
			indexMin = i
			rMin = r
		end # if
		println("i = $i, eSSQ = $eSSQ, r = $r") 
		println("       yHat = $yHat")
		println("       y    = $y")
	end # for i
	indexMin, eSSQ_Min, rMin
end # function lookForSíngleBestCategory

# ╔═╡ 3b529980-f7d4-4e2c-b902-4d9d8f33ba17
lookForSingleBestCategory(y1', y1ThurstoneModel)

# ╔═╡ fb01e2b3-dbaf-4ed5-8737-6f871aeff40f
modelComparison1("cat 7, eSSQ =  0.033, r = 0.7896", [0.42578140000000003, 0.38707400000000003, 0.0774148], y1ThurstoneModel; xlabel="Regression", ylabel="Thurstone Model")

# ╔═╡ 7f1bf0d9-47d6-4bf9-9d8d-c2e83e5eec53
md"
---
###### OLS-Regression with 2 Most Important Regressors
Extract pair of columns of y1 with smallest eSSQ. This happens for the combination of the *4th* and the *8th* category. This generates nearly exact predictions: the multiple $R(y, yHat)=0.99688.$ with an $eSSQ=0.000228298.$ It is interesting that

- category $4$ correlates *negative* with the predictions of Thurstone's model: $r=-0.8737$
The order of regression predictions is *inverse* to Thurstone model predictions:

$S1<S2<S3$
$\;$

- category $8$ correlates *positive* with the predictions of Thurstone's model: $r=0.9891$

The order of regression predictions is *identical* to Thurstone model predictions:

$S3<S2<S1.$

As *single* predictors each is only good but not perfect but *together* they combine perfect.
"

# ╔═╡ f4baecd3-99ff-4cef-8103-8e19d52012b2
function lookForPairOfBestCategories(X, y)
	nS, nCats = size(X)
	xi = zeros(Float64, (nS, 2))
	eSSQ_Min = 1.0; indexI_Min = 0; indexJ_Min = 0; rMin = 0.0
	for i in 1:nCats
		for j in i+1:nCats
			for k in 1:nS
		 		xi[k, 1] = X[k, i] # copy column i out of X
				xi[k, 2] = X[k, j] # copy column j out of X
			end # for k
			β, yHat, sum(yHat), eSSQ, r = firstChoiceRegression(xi, y)
			if eSSQ < eSSQ_Min
				eSSQ_Min = eSSQ
				indexI_Min = i; indexJ_Min = j
				rMin = r
			end # if
			println("i=$i,j=$j, eSSQ = $eSSQ, r = $r") 
			println("         yHat = $yHat")
			println("         y    = $y")
		end # for j
	end # for i
	indexI_Min, indexJ_Min, eSSQ_Min, rMin
end # function lookForPairOfBestCategories

# ╔═╡ a84e8ab7-0935-4dd0-976a-59f861abed0d
lookForPairOfBestCategories(y1', y1ThurstoneModel)

# ╔═╡ 44709a59-eb71-49e0-a2a0-82c9bf3ca3ae
modelComparison1("cats 4 and 8, eSSQ =  0.0002, r = 0.9968", [0.4813582385490143, 0.2882488084460912, 0.235231018599332], y1ThurstoneModel; xlabel="Regression", ylabel="Thurstone Model")

# ╔═╡ be160724-356f-4fb9-8ff7-14049d7293a8
md"
---
###### Thurstone's 2nd Dataset
"
...
y2ThurstoneModel = [0.472173, 0.299155, 0.228672]
...
- for category 16: $eSSQ = 0.0532$ and *positive* single $r(y, yHat)=0.8865$ 
...
lookForSingleBestCategory(y2', y2ThurstoneModel)

# ╔═╡ f4ab6423-ad57-4876-8b9a-a173dee8b570
modelComparison1("cat 16, eSSQ = 0.0532, r = 0.8865", [0.45414732432432436, 0.324390945945946, 0.0], y2ThurstoneModel; xlabel="Regression", ylabel="Thurstone Model")

# ╔═╡ efb3eeca-e169-4473-bec1-68bc3e710062
md"
---
###### OLS-Regression with 2 Most Important Regressors
Extract pair of columns of y1 with smallest eSSQ. This happens for the combination of the *8th* and the *16th* category. This generates nearly exact predictions: the multiple $R(y, yHat) = 0.9999.$ with an $eSSQ=7.70970002120976e-7.$ It is interesting that

- category $8$ correlates *negative* with the predictions of Thurstone's model: $r = -0.5035$
The order of regression predictions is *different* from the Thurstone model predictions:

$S2<S1<S3$

- category $16$ correlates *positive* with the predictions of Thurstone's model: $r = 0.8865$
...
lookForPairOfBestCategories(y2', y2ThurstoneModel)

# ╔═╡ 47d4c770-bf1a-4450-a0c8-d31bac444d03
modelComparison1("cats 8, 16, eSSQ = 7.70970002120976e-7, r = 0.9999", [0.4726784612937433, 0.2984473541887592, 0.22855068928950154], y2ThurstoneModel; xlabel="Regression", ylabel="Thurstone Model")

# ╔═╡ 027c3cdb-a4bd-4760-9bcb-ab501fa8b48c
md"
---
###### Civil Courage Ratings
"
...
yS1 = 	[.33, .10, .05, .00, .28, .24]                     # empirical voting data  

# ╔═╡ 31e42220-7ce7-4681-9bc3-39ee0b08dd5e
md"
###### OLS-Regression with Single Most Important Regressor
Extract 1 column of S1 with smallest error sum-of-squares $eSSQ$. This generates good predictions: 

- for category 5: $eSSQ = 0.0570955$ and *positive* single $r(y, yHat)=0.611907$ 
...
lookForSingleBestCategory(S1', yS1)

# ╔═╡ 9df0d4d2-366b-42e2-8d0a-ac442733cde8
dataModelComparison(title="Data-Model-Comparison:Civil Courage; cat 5",
	[0.1474322580645161, 0.09659354838709676, 0.16776774193548386, 0.09659354838709676, 0.28978064516129026, 0.21860645161290318], # Regression Model
...
---
###### OLS-Regression with 2 Most Important Regressors
Extract pair of columns of y1 with smallest eSSQ. This happens for the combination of the *4th* and the *5th* category. This generates only good predictions: the multiple $R(y, yHat) = 0.7585.$ with an $eSSQ=0.0438.$
"

# ╔═╡ 69c5aabd-33cb-47b6-aa03-6c82bc098c97
lookForPairOfBestCategories(S1', yS1)

# ╔═╡ bfbbb36d-81f0-471d-be86-2a8e6b878631
dataModelComparison(title="Data-Model-Comparison:Civil Courage; cats 4, 5",
	[0.1964548858616474, 0.10065657098720593, 0.11643206304890348, 0.13427649961292198, 0.23472985571018568, 0.27947023517477704], # Regression Model
	[.33, .10, .05, .00, .28, .24],                                 # empirical voting
	xlabel="Regression Model")
...
yS2 = 	[.33, .10, .05, .00, .28, .24]                     # empirical voting data
...
- for category 6: $eSSQ = 0.0132$ and *positive* single $r(y, yHat)=0.9694$ 
...
lookForSingleBestCategory(S2', yS2)

# ╔═╡ c5016fea-b5f8-4a67-b269-8db42badecaf
dataModelComparison(title="Data-Model-Comparison: Liberality; cat 6",
	[0.2814353395332515, 0.15708018950693106, 0.12435515002632043, 0.03272503948061064, 0.24871030005264086, 0.24871030005264086], # Regression Model
	[.33, .10, .05, .00, .28, .24],                                 # empirical voting
	xlabel="Regression Model")
...
md"
###### OLS-Regression with 2 Most Important Regressors
Extract pair of columns of y1 with smallest eSSQ. This happens for the combination of the *4th* and the *6th* category. This generates perfect predictions: the multiple $R(y, yHat) = 0.9903.$ with an $eSSQ=0.0019.$
"

# ╔═╡ 3279576b-9fca-4e65-8002-61c54efcd4be
lookForPairOfBestCategories(S2', yS2)

# ╔═╡ 1a8794d6-97cf-4435-8b5d-a85c2e02f7a3
dataModelComparison(title="Data-Model-Comparison: Liberality; cats 4, 6",
	[0.31464918665116204, 0.10043881815274505, 0.0758176784179184, 0.006878487160403755, 0.3042221689758737, 0.2190574366186438],  # Regression Model
	[.33, .10, .05, .00, .28, .24],                                 # empirical voting
	xlabel="Regression Model")
...
yS3 = 	[.33, .10, .05, .00, .28, .24]                     # empirical voting data
...
- for category 6: $eSSQ = 0.0124$ and *positive* single $r(y, yHat)=0.9291$ 
...
lookForSingleBestCategory(S3', yS3)

# ╔═╡ 4461c98e-4a23-4cef-b02a-34672fd0993c
dataModelComparison(title="Data-Model-Comparison: Honesty; cat 6",
	[0.2459676945668135, 0.16115124816446402, 0.042408223201174745, 0.0, 0.27989427312775333, 0.27989427312775333],                      # Regression Model
	[.33, .10, .05, .00, .28, .24],                                 # empirical voting
	xlabel="Regression Model")
...
md"
###### OLS-Regression with 2 Most Important Regressors
Extract pair of columns of y1 with smallest eSSQ. This happens for the combination of the *6th* and the *7th* category. This generates perfect predictions: the multiple $R(y, yHat) = 0.9665$ with an $eSSQ=0.0060.$
"

# ╔═╡ 8f2f08ad-8a4f-4aa7-bb2b-8050127ef861
lookForPairOfBestCategories(S3', yS3)

# ╔═╡ 9a245e3f-abb8-4685-97b6-99fdbf768f72
dataModelComparison(title="Data-Model-Comparison: Honesty; cat 6, 7",
	[0.29775819607311493, 0.12143789963623218, 0.06153776633001194, 0.0, 0.24049888158340674, 0.29374364536026565],                      # Regression Model
	[.33, .10, .05, .00, .28, .24],                                 # empirical voting
	xlabel="Regression Model")
...
yS4 = 	[.33, .10, .05, .00, .28, .24]                     # empirical voting data

# ╔═╡ 46513a43-666a-4373-950a-de312c036131
md"
###### OLS-Regression with Single Most Important Regressor
Extract 1 column of S1 with smallest error sum-of-squares $eSSQ$. This generates bad predictions: 

- for category 5: $eSSQ = 0.0757$ and *positive* single $r(y, yHat)=0.4067$ 
...
lookForSingleBestCategory(S4', yS4)

# ╔═╡ 168712bf-88b6-4a85-93d3-99de262a8ee4
dataModelComparison(title="Data-Model-Comparison:Intelligence; cat 5",
	[0.1444259385665529, 0.11952491467576792, 0.21414880546075085, 0.09462389078498294, 0.21414880546075085, 0.21414880546075085], # Regression Model
	[.33, .10, .05, .00, .28, .24],                                 # empirical voting
	xlabel="Regression Model")
...
md"
###### OLS-Regression with 2 Most Important Regressors
Extract pair of columns of y1 with smallest eSSQ. This happens for the combination of the *4th* and the *5th* category. Even this combination generates bad predictions: the multiple $R(y, yHat) = 0.7785$ with an $eSSQ=0.0371.$
"

# ╔═╡ 7d322c5d-a484-4dbb-ad37-0b728584bcf0
lookForPairOfBestCategories(S4', yS4)

# ╔═╡ 29c9f6f2-e1eb-40ac-a1a7-049717b5a1e3
dataModelComparison(title="Data-Model-Comparison:Intelligence; cats 4,5",
	[0.283598914928128, 0.050393408468035195, 0.21343199423905146, -0.025607241736114894, 0.21343199423905146, 0.21343199423905146], # Regression Mod
	[.33, .10, .05, .00, .28, .24],                                 # empirical voting
	xlabel="Regression Model")
...
yS5 = 	[.33, .10, .05, .00, .28, .24]                     # empirical voting data
...
- for category 6: $eSSQ = 0.0370$ and *positive* single $r(y, yHat)=0.7704$ 
...
lookForSingleBestCategory(S5', yS5)

# ╔═╡ a92df357-86cb-42bb-bef1-6c79e0edff2f
dataModelComparison(title="Data-Model-Comparison: Objectivity; cat=6",
	[0.18615744203440537, 0.21183433059087511, 0.06419222139117428, 0.03209611069558714, 0.24393044128646224, 0.2760265519820494],  # Regression Model
	[.33, .10, .05, .00, .28, .24],                                 # empirical voting
	xlabel="Regression Model")
...
md"
###### OLS-Regression with 2 Most Important Regressors
Extract pair of columns of y1 with smallest eSSQ. This happens for the combination of the *6th* and the *7th* category. This generates perfect predictions: the multiple $R(y, yHat) = 0.9128$ with an $eSSQ=0.0244.$
"

# ╔═╡ a3c4d037-0a67-40e2-8c08-e09ab9ff945a
lookForPairOfBestCategories(S5', yS5)

# ╔═╡ c2694462-a1be-431a-a7f5-c61df42595b9
dataModelComparison(title="Data-Model-Comparison: Objectivity, cats 6,7",
	[0.26673164093680474, 0.18427131417605, 0.12257248518197723, 0.05059605390533449, 0.2081418963672492, 0.25873795027258367],                       # Regression Mod
	[.33, .10, .05, .00, .28, .24],                                 # empirical voting
	xlabel="Regression Model")
...
$\begin{array}{c|c|c}
\text{scale} & \text{cat} & \text{eSSQ for 1 cat} & \text{r for 1 cat} & \text{cats}  & \text{eSSQ for 2 cats} & \text{r for 2 cats}  \\

\hline \\

\text{Civil Courage} & 5 & 0.0570955 & 0.611907 & 4, 5 & 0.0438854  & 0.758557 \\
\mathbf{Liberality}  & \mathbf 6 & \mathbf{0.0132712} & \mathbf{0.969498} & \mathbf{4, 6} & \mathbf{0.00197501} & \mathbf{0.990344} \\
\mathbf{Honesty} &  \mathbf 6 & \mathbf{0.0124501} & \mathbf{0.929141} & \mathbf{6, 7} & \mathbf{0.00608096} & \mathbf{0.966582} \\
\text{Intelligence}  & 5 & 0.0757221 & 0.406776 & 4, 5 & 0.0371168  & 0.778516 \\
\text{Objectivity}   & 6 & 0.0370281 & 0.770441 & 6, 7 & 0.024446   & 0.912818 \\

\end{array}$

The table (above) demonstrates that the most informative categories $4,5,6$ don't lay in the extremes but in the moddle of the rating scales. The best predictor is *Liberality* followed by *Honesty*. These results are congruent to the results of the *Voting* model.
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "112515c5a1573fb4fb6be6da801a17bc01fa5da9"
