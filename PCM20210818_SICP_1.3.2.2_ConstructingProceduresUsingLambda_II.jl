...
# ╔═╡ b00c8772-b153-460c-a559-bd8ee99f93d2
begin 
	using Pluto
	using Plots	
	using LaTeXStrings
	...
	# println("pkgversion(QuadGK)        = ", pkgversion(QuadGK))
end # begin

# ╔═╡ 0bdc1f22-be80-11ed-1491-69cfe2382d1b
md"
=====================================================================================
#### SICP: 1.3.2.2 Constructing Procedures Using Lambda II: Ωmega & CPS
##### file: PCM20210818\_SICP\_1.3.2.2\_ConstructingProceduresUsingLambda\_II.jl
##### Julia/Pluto.jl-code (1.11.4/0.20.4) by PCM *** 2025/04/02 ***

=====================================================================================
"

# ╔═╡ 7dfe0d0d-91d9-4d5c-9b2b-2244a7a2d106
md"
##### 0. Introduction

If we want to define *adhoc* functions which are *anonymous* and have *no* name we can do this by using the concept $lambda$. This is a *computer science (CS)* concept first defined in the [*Lambda Calculus*](https://en.wikipedia.org/wiki/Lambda_calculus) and *not* identical with the *mathematical measure-theoretic* concept of *Lebesgue*'s $\lambda$. The CS $lambda$ is also a *keyword* in Scheme and Haskell denoting the definition of an *anonymous* function. In Julia $lambda$ is not used explicitly but a '$->$' is used instead.

In [*lambda calculus*](https://plato.stanford.edu/entries/lambda-calculus/) we find the so called Ω-*combination*. This is a self-replicating function call of a λ-expression. This can be used as a test whether the computer language has *tail-call optimization (TCO)*. This is the case for Scheme but *not* for Julia and even *not* for Haskell. 

A further use of λ-expressions is that they can represent *continuations*. *In computer science, a [continuation](https://en.wikipedia.org/wiki/Continuation) is an abstract representation of the control state of a computer program. A continuation implements (reifies) the program control state, i.e. the continuation is a data structure that represents the computational process at a given point in the process's execution; the created data structure can be accessed by the programming language, instead of being hidden in the runtime environment. Continuations are useful for encoding other control mechanisms in programming languages such as exceptions, generators, coroutines, and so on.*([Wikipedia](https://en.wikipedia.org/wiki/Continuation)) 

"

# ╔═╡ bbc9d2e8-95d7-44da-a9cc-2ecb485364fb
md"
---
##### 1. Topics
- *anonymous* function
...
- *keyword* $lambda$, Julia's '$->$'
- Ω-*combinator*, *Ω-combination*
- *Contination*
...
# ╔═╡ 1e1b9bfb-d682-4718-9523-5c0b33a58824
md"
---
##### 2. Packages
"
...
md"
---
##### 3.1 nonSICP: Ω-Cmbinator and Ω-Combination
...
###### in [*Scheme*](https://en.wikipedia.org/wiki/Scheme_(programming_language)) or [*Racket*](https://racket-lang.org/):
Scheme with *tail-call optimization* evaluates this *lambda*-expression
...
Julia without *tail-call optimization* evaluates this *lambda*-expression 

$(x -> x(x))(x -> x(x))$

$\;$

till *stack overflow* stops evaluation:
...
try
    ...
    @warn "no tco, not terminating lambda-expression"
end

# ╔═╡ 0ca79582-2627-4e3e-a827-14cd9acd1b28
(x -> x(x))(x -> x(x))

# ╔═╡ 0013515a-fc8a-45df-8c08-3c7b7de17cb8
md"
---
##### 3.2 CPS (= [Continuation Passing Style](https://en.wikipedia.org/wiki/Continuation-passing_style))

*In functional programming, continuation-passing style (CPS) is a style of programming in which control is passed explicitly in the form of a continuation. This is contrasted with direct style, which is the usual style of programming. Gerald Jay Sussman and Guy L. Steele, Jr. coined the phrase in AI Memo 349 (1975), which sets out the first version of the Scheme programming language... John C. Reynolds gives a detailed account of the numerous discoveries of continuations...* (Wikipedia, [*CPS*](https://en.wikipedia.org/wiki/Continuation-passing_style#cite_note-2))
"

# ╔═╡ e230ec99-56b8-416a-9f47-9feaf46dab1b
md"
---
###### 3.2.1 Basic CPS-Operators
"

# ╔═╡ 857a21e8-dc2f-415c-9669-aff2e1b30343
cpsAdd(x, y, k)  = k(+(x, y))                  # k is a continuation (function)

# ╔═╡ 70cec50e-6c07-460c-99a0-e3e9c582b155
cpsSub(x, y, k)  = k(-(x, y))                  # k is a continuation (function)

# ╔═╡ 8dde94f4-4585-404c-ab33-eba1e1803551
cpsAddExtended(x, y, k)  = k(x, y, +(x, y))    # leaves args x and y untouched

# ╔═╡ 63d8e961-e14b-4c41-9169-496b41819fa4
cpsSubExtended(x, y, k)  = k(x, y, -(x, y))    # leaves args x and y untouched

# ╔═╡ cb84a330-6c45-4da2-8ac3-a9e843129573
cpsMult(x, y, k) = k(*(x, y))

# ╔═╡ bf0dd2d5-fbab-4435-979a-89c2b52297c8
cpsMultExtended(x, y, k) = k(x, y, *(x, y))

# ╔═╡ dcd3061b-b54b-41a0-a218-1304c2559b1b
cpsExp(x, y, k)  = k(^(x, y))

# ╔═╡ 5bf04fcc-7538-4f42-a54a-88f7bafd280c
cpsExpExtended(x, y, k)  = k(x, y, ^(x, y))

# ╔═╡ 697d7b63-cba6-4569-950e-10d3f7bbdff1
cpsEq(x, y, k)   = k(==(x, y))

# ╔═╡ e8b8ce85-2620-4c4f-89e9-62202af0a837
cpsEqExtended(x, y, k)   = k(x, y, ==(x, y))

# ╔═╡ 77265238-4164-44f1-ab49-7d6c912385f0
cpsIfThenElse(expr1, expr2, condition, kTrue, kFalse) = 
    let booleanValue = condition(expr1, expr2)
		booleanValue ? 
			kTrue(booleanValue) : # application of true-Continuation
			kFalse(booleanValue)  # application of false-Continuation
	end # let

# ╔═╡ 0c8aad6a-aba5-40b7-ae0e-539f2ef58c8d
cpsIfThenElseExtended(expr1, expr2, condition, kTrue, kFalse) =
	let booleanValue = condition(expr1, expr2)
		booleanValue ? 
			kTrue(expr1, expr2, booleanValue) : # application of true-Continuation
			kFalse(expr1, expr2, booleanValue)  # application of false-Continuation
	end # let

# ╔═╡ 22c29e5f-9512-4460-9678-22f3d3efb4df
md"
---
###### 3.2.2 Tests (= Applications) of Basic CPS-Operators
"
...
# ╔═╡ 76a6f951-387d-4ce6-b81f-358edfad38a0
cpsAddExtended(3, 4, (x, y, xAddY) -> print("$x + $y = $xAddY"))

# ╔═╡ 9fd6c383-c3b8-4b8b-957f-b8e458054a3d
cpsSub(1, 2, xSubY -> print("x - y is $xSubY"))

# ╔═╡ 9f87ec5b-8e4c-4a95-b18d-fce77d59b1cd
cpsSubExtended(3, 4, (x, y, xSubY) -> print("$x - $y = $xSubY"))

# ╔═╡ 12a0195e-3038-4f1d-968a-9631b59c41f4
cpsMult(3, 4, xMultY -> print("x * y is $xMultY"))

# ╔═╡ 63d00764-4c39-493b-a522-efe1cbeb7cd0
cpsMultExtended(3, 4, (x, y, xMultY) -> print("$x * $y is $xMultY"))

# ╔═╡ 8353921c-ec1c-4896-9ffa-91e06009a725
cpsExp(3, 4, xExpY -> print("x^y is $xExpY"))

# ╔═╡ 7534cd19-119c-460f-b16a-82594647bd1d
cpsExpExtended(3, 4, (x, y, xExpY) -> print("$x^$y is $xExpY"))

# ╔═╡ a09663da-e08d-4866-9927-4fea1c9ac11c
cpsEq(1, 1, xEqY -> print("x == y is $xEqY"))

# ╔═╡ 3787770c-fd09-41fe-8132-ae9a691f992e
cpsEqExtended(1, 1, (x, y, xEqY) -> print("$x == $y is $xEqY"))

# ╔═╡ 28ecdc1f-291f-4dca-a0fa-22d66fef2fc2
cpsEq(1, 2, xEqY -> print("x == y is $xEqY"))

# ╔═╡ aacd5a00-e178-461a-a020-3e36da97baf6
cpsEqExtended(1, 2, (x, y, xEqY) -> print("$x == $y is $xEqY"))

# ╔═╡ 1f35d87a-cb9a-4006-b48c-112122d0b744
cpsIfThenElse( 0, 0, ==, 
	booleanValue -> 
		print("for expr1 and expr2 is condition = $booleanValue"),  # true-contin.
	booleanValue -> 
		print("for expr1 and expr2 is condition = $cbooleanValue")) # false-contin.

# ╔═╡ 9022f7cd-0bde-4de2-ae72-c124582e139e
cpsIfThenElse( 0, 5, ==, 
	booleanValue -> 
		print("for expr1 and expr2 is booleanValue = $booleanValue"), # true-contin.
	booleanValue -> 
		print("for expr1 and expr2 is booleanValue = $booleanValue")) # false-contin.

# ╔═╡ dbfffd66-2ecb-4e8c-89ff-cf6e635f2ac9
cpsIfThenElseExtended( 0, 0, ==, 
	(expr1, expr2, condition) -> 
		print("for $expr1 and $expr2 is condition = $condition"), # true-contin.
	(expr1, expr2, condition) -> 
		print("for $expr1 and $expr2 is condition = $condition")) # false-contin.

# ╔═╡ c1637306-5454-4e24-ac87-4555abebd7c7
cpsIfThenElseExtended( 0, 5, ==,
	(expr1, expr2, condition) -> 
		print("for $expr1 and $expr2 is condition = $condition"), # true-contin.
	(expr1, expr2, condition) -> 
		print("for $expr1 and $expr2 is condition = $condition")) # false-contin.
...
md"
---
##### 3.3 Examples: CPS-Style Functions
Here are some examples contrasting [*direct* and CPS-style](https://en.wikipedia.org/wiki/Continuation-passing_style).
"

# ╔═╡ 2c4031b1-b1f2-47fc-ba72-7a3390445c79
md"
---
##### 3.3.1 Nonrecursive Example: *Pythagoras*
...
md"
---
###### 3.3.3.1 DST (=*Direct* STyle)
"

# ╔═╡ e0f8d9f6-62a2-47d4-9fac-10672738b90b
function pythagorasDSTPrefix(a, b)                 # DSt = Direct Style
	^( +( *( a, a), *( b, b)), 1/2)                # exponentiation
end # function pythagorasDStPrefix

# ╔═╡ b850208d-49d7-489c-9867-2acbd366041d
let a = 3
	b = 4
	c = pythagorasDSTPrefix(a, b)
end # let

# ╔═╡ 907d1a55-e536-4fe1-83e2-8c67ed942ddd
function pythagorasDSTInfix(a, b)                 # DSt = Direct Style
	sqrt(a^2 + b^2)                               # sqrt
end # function pythagorasDStInfix

# ╔═╡ 91c2430c-7bc0-458f-99d6-f93cc6d60a9c
let a = 3
	b = 4
	c = pythagorasDSTInfix(a, b)
end # let

# ╔═╡ eaad7fad-c313-482f-920d-5d2885876ee7
md"
---
###### 3.3.3.2 ISS (= Imperative Sequential Style)
"
...
md"
---
###### 3.3.3.4 NLS (= Nested Lambdas Style)
"
...
md"
---
###### 3.3.3.5 CPS (= Continuation Passing Style)
"
...
md"
---
##### 3.3.2 Linear Recursive Example: Factorial
...
md"
---
###### 3.3.2.2 ISS (= Imperative Sequential Style)
"
...
md"
---
###### 3.3.2.3 CPS-(= Continuation Passing)-Style

###### 3.3.2.3.1 $factorialCPS1$ is a mixture of *DST* and *CPS*-Style:
...
md"
---
###### 3.3.2.3.2 NLS (Nested Lambda Style)): $factorialCPS2$ 
"

# ╔═╡ 2d7e5950-a472-4ff8-80d6-317a8cec04be
function factorialNLS2(n)
	let isEqZeroN = n == 0                # local definition
		let                               # empty local binding
			isEqZeroN ? 1 : 
				let                       # empty local binding
					nSub1 = n - 1
...
md"
---
###### 3.3.2.3.3 $factorialCPS2$ is a *pure* CPS-styled Recursive Factorial
The condition $if...then...else$ has been replaced by the function $cpsIfThenElse$ with two continuations $trueContinuation$ and $falseContinuation.$

"

# ╔═╡ e370bc78-e2ce-4db7-aefb-5678c9080977
function factorialCPS2(n, k; facValNSub1=1)
	cpsIfThenElse(n, 1, <=, 
		stopCondition -> k(facValNSub1),                 # true continuation 
		stopCondition ->                                 # false continuation
			cpsSub(n, 1, nSub1 -> 
				factorialCPS2(nSub1, 
					facValNSub1 -> 
						cpsMult(n, facValNSub1, k))))
...
# ╔═╡ 84f5059d-b204-4e66-9ba5-fe46f83dce85
factorialCPS2(2, result -> result)
...
md"
---
##### 3.3.3 Linear Iterative Example: *Tail-Recursive* Factorial
...
md"
---
###### 3.3.3.2 CPS-(= Continuation Passing)-Style

###### 3.3.3.2.1 $factorial2CPS1$ is a mixture of *DST* and *CPS*-Style:
...
md"
---
###### 3.3.3.2.2 $factorial2CPS2$ is a *pure* CPS-styled Recursive Factorial
The condition $if...then...else$ has been replaced by the function $cpsIfThenElse$ with two continuations $trueContinuation$ and $falseContinuation.$

"

# ╔═╡ e27cdd55-4541-4b72-84d8-9a262b7f1783
function factorial2CPS2(n, k)
	function iter(product, counter, k)
		cpsIfThenElse(counter, 1, <=, 
				stopCondition ->                               # true continuation
					k(product), 
				stopCondition ->                               # false continuation
					cpsSub(counter, 1, counterSub1 ->    
								cpsMult(counter, product, countMultProd -> 						iter(countMultProd, counterSub1, k))))
...
# ╔═╡ 87c89897-1823-4d7d-bd7e-00dad7dc672a
md"
---
##### 5. Summary

We defined the Ω-*combinator* and Ω-*combination* in Scheme and Julia. Wheras Scheme has no problem with running the Ω-*combination* Julia has. Julia runs into a *stack-overflow* because it has no *tco*.

Then we defined some example problems in a *CPS*. First a *nonrecursive* (Pythagoras), then a *linear recursive* (factorial) and a *tail-recursive* (factorial).
...
- **Abelson, H., Sussman, G.J. & Sussman, J.**; [*Structure and Interpretation of Computer Programs*](https://web.mit.edu/6.001/6.037/sicp.pdf): 2016; last visit 2025/03/09

- **Reynolds, J.C.**; [*The discoveries of continuations*](https://link.springer.com/article/10.1007/BF01019459); LISP and Symbolic Computation 6, 233–247 (1993); last visit 2025/04/01

- **Stanford Encyclopedia of Philosophy**; [*The Lambda Calculus*](https://plato.stanford.edu/entries/lambda-calculus/); last visit 2025/04/01

- **Stark, P.A.**; *Introduction to Numerical Methods*, NY: Macmillan Company, 1970

- **Wikipedia**; [*Continuation*](https://en.wikipedia.org/wiki/Continuation); last visit 2025/04/01

- **Wikipedia**; [*Continuation Passing Style*](https://en.wikipedia.org/wiki/Continuation-passing_style); last visit 2025/03/12 -

- **Wikipedia**; [*Lambda Calculus*](https://en.wikipedia.org/wiki/Lambda_calculus); last visit 2025/03/31
