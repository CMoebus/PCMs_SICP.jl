...
# ╔═╡ def5f1e0-51a8-4a6a-89e6-023e6a561e54
using Plots

# ╔═╡ 9545cbb0-43f2-11ee-394a-d7a0f311d879
md"
====================================================================================
#### SICP: 3.5.3.3 Streams as Signals
##### file: PCM20230826\_SICP\_3.5.3.3\_Streams\_As\_Signals.jl
##### Julia/Pluto.jl-code (1.9.3/0.19.27) by PCM *** 2023/08/27 ***

====================================================================================

"

# ╔═╡ 78837e5f-1c9f-4674-b932-0ae85dfebd7d
md"
###### Types
"

# ╔═╡ 2bf87628-30c5-4c4e-bcfb-d5de51e32aff
UnOfUnRaTaFiDroRes = Union{UnitRange, Base.Iterators.Take, 					   			Base.Iterators.Filter, Base.Iterators.Drop, Base.Iterators.Rest}

# ╔═╡ e61b5e29-4696-4e4f-8d24-932341592d29
md"
---
###### Constructors
"

# ╔═╡ e879a574-bd1c-4991-a2a5-3dedf67ed241
#---------------------------------------------------
# from ch. 2.2.1
#---------------------------------------------------
struct Cons
...
# ╔═╡ 1cb12f93-0bfe-457b-81d6-a3826a47adb2
cons(car, cdr) = Cons(car, cdr)  

# ╔═╡ 99dcd949-d90c-470d-846a-7ab817152745
md"
---
###### Selector
"

# ╔═╡ 45a13fe4-5141-474a-bad3-fc5a57e6d0b2
#--------------------------------------------------------
# from ch. 2.2.1
#--------------------------------------------------------
car(cons::Cons) = cons.car

# ╔═╡ 60ffcc86-ea5d-4e6c-99db-603397b47d2b
#--------------------------------------------------------
# taken from ch. 2.2.1
#--------------------------------------------------------
cdr(cons::Cons) = cons.cdr

# ╔═╡ 6d21bbee-52fe-4590-8da5-52f0fdafc0a4
begin
...
# ╔═╡ 75c31282-4819-4c2f-96b2-956b7acd2b05
streamNull = null   

# ╔═╡ a278af7c-3da5-4c85-9133-1b27bb174b04
md"
---
###### Functors
"

# ╔═╡ 8320f6c6-3825-48ed-a31b-a87407df3069
md"
---
###### Stream Functions
"

# ╔═╡ b946bf9b-26f0-4d2e-ba86-3be548146c38
#-----------------------------------------------------------------------------------
...
# ╔═╡ 8f63db29-0487-4ac3-bdc7-446db7bc82e9
begin
	#===============================================================================#
...
# ╔═╡ 38ad388e-39df-443d-9bf7-185cea1f0419
begin                                                            # from 3.5.2.1
	#-------------------------------------------------------------------------------
...
# ╔═╡ c52399b6-f4a5-4184-923d-fc48dd65a573
begin
	#-------------------------------------------------------------------------------
...
# ╔═╡ acedc63c-99d8-408c-8f54-900c03e85bda
begin
	#--------------------------------------------------------------------------------
...
# ╔═╡ 485fd850-c7db-46ae-9ebf-a0171eb9007a
function streamAdd(stream1, stream2)
	streamMap(+, stream1, stream2)
end # function streamsAdd

# ╔═╡ 635562f2-b64c-44d4-baa5-f8e1768fb03b
function streamScale(stream::Cons, scaleFactor::Real)            # SICP, p.329
	streamMap(x -> x * scaleFactor, stream)
end # function streamScale

# ╔═╡ 4f3eb535-7842-4174-b541-98f434eb7860
md"
---
###### Applications
"

# ╔═╡ f1455354-412a-4a84-90f8-4568cc7d78cc
md"
---
*An integrator or summer accumulates the $sum(i)$ for an input stream $x \equiv (x_i)$, an initial value $C$, and a small increment $dt$* (SICP, p.343)  

$S_i = C + \sum_{j=1}^i x_j \cdot Δx$ ($dt$ replaced here by $\Delta$*x*)

$\;$
"

# ╔═╡ 35ab6677-485d-4051-9274-5610df2640d5
function integral(integrand::Cons, initialValue::Real, dt::Real)        # SICP, p.343
	integral = streamCons(initialValue,
			() -> streamAdd(streamScale(integrand, dt), integral))
	integral
end # function integral

# ╔═╡ 6e0055f4-0db3-4492-8e5d-3e871c1c574c
md"
---

$\;$

$\int_0^1 x \; dx \Longrightarrow \left . \frac{x^2}{2} \right |_0^1 = \Bigg [\frac{1}{2} - \frac{0}{2} \Bigg ] = \frac{1}{2} = 0.50$
...
###### input stream $x \equiv(x_i)$

$\;$

"

# ╔═╡ 18ccecdd-8718-41ea-b93d-44416344508c
plot(x -> x, 0.0, 1.0, size=(700, 300), xlims=(0.0, 1.1), ylims=(0, 1.2), line=:darkblue, fill=(0, :lightblue), title="x->x", framestyle=:semi)

# ╔═╡ 1ea36e8f-e452-4a63-9654-41400940aaca
let Δx = 0.01
	streamX  = streamCons(Δx, () -> streamX)::Cons
	streamXi = streamCons(0.0, () -> streamAdd(streamX, streamXi))
	integralStream = integral(streamXi, 0.0, Δx)
	streamRef(integralStream, convert(Int64, round(1.0/Δx)))
end # let

# ╔═╡ 925f31dc-d3d5-4e92-ab0c-17fb28f93378
let Δx = 0.001
	streamX  = streamCons(Δx, () -> streamX)::Cons
	streamXi = streamCons(0.0, () -> streamAdd(streamX, streamXi))
	integralStream = integral(streamXi, 0.0, Δx)
...
# ╔═╡ 9d9a387c-2710-4522-aece-6995afc9cc19
md"
---

$\;$

$\int_0^1 x^2 \; dx \Longrightarrow \left . \frac{x^3}{3} \right |_0^1 = \Bigg [\frac{1}{3} - \frac{0}{3} \Bigg ] = \frac{1}{3} = 0.333...$
...
###### input stream $x^2 \equiv(x_i^2)$

$\;$

"

# ╔═╡ 80637990-afd7-484e-b459-0d49b6bb58bf
plot(x -> x*x, 0.0, 1.0, size=(700, 300), xlims=(0.0, 1.1), ylims=(0, 1.2), line=:darkblue, fill=(0, :lightblue), title="x->x^2", framestyle=:semi)

# ╔═╡ 254ed6e9-3257-420c-baf9-646fd6cc458a
let Δx = 0.01
	streamX  = streamCons(Δx, () -> streamX)::Cons
	streamXi = streamCons(0.0, () -> streamAdd(streamX, streamXi))
	streamXiSquare = streamMap(xi -> xi*xi, streamXi)::Cons
	integralStream = integral(streamXiSquare, 0.0, Δx)
	streamRef(integralStream, convert(Int64, round(1.0/Δx)))
end # let

# ╔═╡ d8493ad0-499a-4964-b443-fc5c7cd033f1
let Δx = 0.001
	streamX  = streamCons(Δx, () -> streamX)::Cons
	streamXi = streamCons(0.0, () -> streamAdd(streamX, streamXi))
	streamXiSquare = streamMap(xi -> xi*xi, streamXi)::Cons
...
# ╔═╡ 9494ea94-3c3d-4d01-b745-d12772a3d7d2
md"
---

$\;$

$\int_0^1 x^3 \; dx \Longrightarrow \left . \frac{x^4}{4} \right |_0^1 = \Bigg [\frac{1}{4} - \frac{0}{4} \Bigg ] = \frac{1}{4} = 0.25$
...
###### input stream $xCube \equiv(x_i^3)$
compare the result of the *stream* approach with that of the *classical* approach (SIPC, ch. 1.3.1, p.60)
...
# ╔═╡ beb660c1-c943-4b44-a89d-0429334cf688
plot(x -> x*x*x, 0.0, 1.0, size=(700, 300), xlims=(0.0, 1.1), ylims=(0, 1.2), line=:darkblue, fill=(0, :lightblue), title="x->x^3", framestyle=:semi)

# ╔═╡ 8e83dea9-170b-4427-b0b3-e9320c54a8a5
let Δx = 0.01                                                   # SICP, p.343
	streamX  = streamCons(Δx, () -> streamX)::Cons
	streamXi = streamCons(0.0, () -> streamAdd(streamX, streamXi))
	streamXiCube = streamMap(xi -> xi*xi*xi, streamXi)::Cons
	integralStream = integral(streamXiCube, 0.0, Δx)
	streamRef(integralStream, convert(Int64, round(1.0/Δx)))
end # let

# ╔═╡ 4ae06f2d-26f5-4472-b7f0-c69ad0eaf746
let Δx = 0.001                                                  # SICP, p.343
	streamX  = streamCons(Δx, () -> streamX)::Cons
	streamXi = streamCons(0.0, () -> streamAdd(streamX, streamXi))
	streamXiCube = streamMap(xi -> xi*xi*xi, streamXi)::Cons
	integralStream = integral(streamXiCube, 0.0, Δx)
...
# ╔═╡ 793da643-694b-46c2-957c-49c0330f5848
md"
---
##### End of ch. 3.5.3.3
"

# ╔═╡ d1aafe8c-8303-494c-a31f-c22e9781d6ce
md"
====================================================================================
