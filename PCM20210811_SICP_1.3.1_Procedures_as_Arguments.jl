...
cube(x) = *(x, x, x)              # '*' here as Scheme-like prefix operator
...
sumCubes2(a, b) = sum1(cube, a, inc, b)
...
sum_integers2(a, b) = sum1(identity, a, inc, b)
...
# ╔═╡ bff6a72e-fa3f-4443-8441-063cfaaea124
sum_integers2(2, 10)
...
###### ... with *abstract* types 'Signed', 'Integer', *self-defined* 'FloatOrSigned', 'while', '+='
" 
...
# ╔═╡ f364a443-034c-473c-af6e-032aac621fa7
supertype(Irrational)

# ╔═╡ 9fde9515-3848-44c1-b33b-de9bae9c77c6
subtypes(Irrational)

# ╔═╡ 456880e2-fe92-4df4-8cb0-3d70ed66a7ce
FloatOrSigned = Union{AbstractFloat, Signed}

# ╔═╡ 890ab8e8-12df-4975-90a4-fda312d60ba2
# idiomatic Julia-code with 'Signed', 'while', '+='
function sumIntegers2(a::Signed, b::Signed)::Signed
	sum = 0
	while !(a > b)
		sum += a
		a += 1
...
# idiomatic Julia-code with 'Signed', 'while', '+='
function sumCubes3(a::Signed, b::Signed)::Signed
	#-------------------------------------------
	cube(x) = x^3
	#-------------------------------------------
	sum = 0
...
# idiomatic Julia-code with 'Signed', 'AbstractFloat' 'while', '+='
function piSum2(a::Signed, b::Signed)::AbstractFloat
	sum = 0
	while !(a > b)
		sum += 1.0 / (a * (a + 2))  
		a += 4
...
# idiomatic Julia-code with 'Function', 'Signed', 'AbstractFloat', 'while', '+='
function sum2(term::Function, a::FloatOrSigned, next::Function, b::FloatOrSigned)::AbstractFloat
	sum = 0
	while !(a > b)
		sum += term(a)
		a = next(a)
...
function piSum3(a::FloatOrSigned, b::FloatOrSigned)::AbstractFloat
	#----------------------------------------------------------------
	piTerm(x) = 1.0 / (x * (x + 2))
	piNext(x) = x + 4
	#----------------------------------------------------------------
	sum2(piTerm::Function, a::FloatOrSigned, piNext::Function, b::FloatOrSigned)
end
...
function integral2(f::Function, a::FloatOrSigned, b::FloatOrSigned, dx::AbstractFloat)::AbstractFloat
	add_dx(x) = x + dx
