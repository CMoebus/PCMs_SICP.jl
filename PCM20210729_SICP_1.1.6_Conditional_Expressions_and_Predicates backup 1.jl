...
===================================================================================
 ...
 ##### Julia/Pluto.jl-code (1.12.5/0.20.3) by PCM *** 2026/03/05 ***
===================================================================================
"
...
md"
##### 0. Introduction
Up to now problem solving and evaluation of expressions could *not* be *situation dependent*. We need constructs controlling the evaluation process. This can be achieved e.g. by a *ternary* *nonstrict* expression: $<predicate>?<expression>:<expression>$. *Nonstrict* means that only *some* arguments of the *operator* are evaluated. All *operators* (like $+, -, *, /,...$) presented before were of the *strict* kind.
...
where *+* is the Kleene repitition operator "at least *one*"}
...
A very simple example for *case-based* problem solving and code is the $abs$-function. This function is *continuous* and *differentiable* everywhere except in $x=0$. The mathematical definition is:

$|x| = \cases{\;\;\; x \;\text{ if } x \gt 0 \cr \;\;\; 0  \;\text{ if } x = 0 \cr -x \;\text{ if } x < 0}$

The Julia-function $my\_abs$ is *generic* (with type Any) and a nearly exact implementation of the *mathematical* definition.
...
md"
---
###### 3.3 *Conditional* Expression $if...elseif...end:$
$if <predicate_1>$ 
$<consequent>^+$ 
$elseif <predicate_2>$ 
$<alternative_1>^+$ 
...
md"
---
###### 3.4 *Binary, Infix, Nonstrict (Shortcut)* Boolean Operator $\&\&$
"
...
md"
###### 3.5 *Binary, Infix, Nonstrict (Shortcut)* Boolean Operator $||$
"
...
md"
###### 3.6 Definition of $geEq$ by using *Prefix* $!$ and *Infix* $<$ 
"

# ╔═╡ 6dc41f11-c3da-46e9-803f-36ae73ea4c9d
md"
###### $geEq1$ based on built-in *infix* $\text{||}, >, ==$ 
"
...
md"
###### 3.7 $geEq$ based on built-in *prefix* $\text{!<}$ 
"
...
md"
###### 4.1 Function $not(<boolean\ expression>)$
"
...
# ╔═╡ e67e681b-cc66-4579-9fa9-a3ebf555094f
md"
---
###### 4.2 *Binary, Prefix, Nonstrict (Shortcut)* Boolean Operator $and(.,.)$
"

# ╔═╡ 8e6999c9-584d-456f-b0b0-9e4a565bba5a
md"
---
###### 4.3 *Binary, Prefix, Nonstrict (Shortcut)* Boolean Operator $or(.,.)$
"

# ╔═╡ 7efcf92c-963e-428e-9393-547bd58a7eea
md"
---
###### 4.4  Boolean *Prefix* Operator $and(.,...)$ based on Julian built-in *Prefix* (*Shortcut*) Function $all$

Julia has unlike Scheme *no prefix* *n-nary* $and(<predicate>,...)$. So we have to define it on basis of *prefix* operator $all(...)$ with *slurping* parameter $preds$
...
# ╔═╡ 929197db-e7dd-45c4-b51d-73323f4db637
and(5 > 2, 10 < 2)                            # ==> and(true, false) ==> false --> :)

# ╔═╡ de16b747-0708-4a22-be40-3d9e271d7454
and(>(5, 2), <(10, 2))                        # ==> and(true, false) ==> false --> :)

# ╔═╡ 5bb2c6f8-32e1-44c7-9c30-d18c91397649
and(5 > 2, 10 > 2)                            # ==> and(true, true) ==> true --> :)

# ╔═╡ 5c4c2145-2121-4dea-8bcd-e1894f77485a
and(>(5, 2), >(10, 2))                        # ==> and(true, true) ==> true --> :)
...
or(5 < 2, 10 < 2)                             # ==> or(false, false) ==> false --> :)

# ╔═╡ 992cf28b-0d37-41ac-8f72-16901f943351
or(5 < 2, 10 > 2)                             # ==> or(false, true) ==> true --> :)
...
manifest_format = "2.0"
project_hash = "e337e1a99294a37bf113fca65201b35c806643df"
...
weakdeps = ["FixedPointNumbers"]
    ...
    RatiosFixedPointNumbersExt = "FixedPointNumber