...
##### Julia/Pluto.jl-code (1.9.3/19.27) by PCM *** 2023/09/10 ***
...
begin
	x = Array{Real, 2}(undef, (9, 12))
	#------------------------------------------------------------------------------
	x[1,:] = [.04, .03, .00, .04, .03, .00, .0000, .0000, .0012, .20, .16, .02]
	x[2,:] = [.16, .13, .02, .20, .16, .02, .0032, .0040, .0620, .29, .31, .16]
	x[3,:] = [.13, .18, .14, .33, .34, .16, .0544, .0528, .1122, .24, .35, .48]
	x[4,:] = [.11, .17, .34, .44, .51, .50, .2550, .2200, .2244, .19, .32, .68]
	x[5,:] = [.08, .15, .34, .52, .66, .84, .5544, .4368, .3432, .14, .27, .48]
	x[6,:] = [.06, .12, .14, .58, .78, .98, .7644, .5684, .4524, .17, .22, .16]
	x[7,:] = [.11, .10, .02, .69, .88, 1.0, .8800, .6900, .6072, .30, .18, .02]
	x[8,:] = [.19, .08, .00, .88, .96, 1.0, .9600, .8800, .8448, .31, .12, .00]
	x[9,:] = [.12, .04, .00, 1.0, 1.0, 1.0, 1.000, 1.000, 1.000, .12, .04, .00]
 	size(x)
end # begin
...
function discreteVotingModel(x::Array, title; k=3, m=9, plimit=1.0, exponent=1)
	let sumOfProbs_ItoK = Array{Real, 1}(undef, k)
		for j in 1:k
			sumOfProbs_ItoK[j] = 0.0
		end # for
		sumOfProbs = 0.0
		while !(sumOfProbs > plimit || m < 1)
			for j in 1:k
				xmj = x[m,j]^exponent                # if exp > 1 then hybrid model
				sumOfProbs_ItoK[j] += xmj
				sumOfProbs += xmj
			end # for
			m -= 1
		end # while
		totalSum = 0.0
		for j in 1:k
			sumOfProbs_ItoK[j] /= sumOfProbs # normalization of probs
			totalSum += sumOfProbs_ItoK[j]
		end # for
		title, m-1, sumOfProbs_ItoK, totalSum
	end # let
end # function discreteVotingModel

# ╔═╡ e9c53501-7ba2-4393-bdee-f609e371a5ac
function modelComparison1(title; xlabel="VotingModel", ylabel="Thurstone Model", x=[], y=[])
...
discreteVotingModel(x, exponent=0, "Thurstone's 1st data set, f(x)^0") # voting model

# ╔═╡ 403c8660-75cb-4866-9a70-e822b425c797
modelComparison1("Thurstone vs Voting Model f(x)^0", x=[0.333333, 0.333333, 0.333333], y=[0.47, 0.30, 0.23]) 
...
discreteVotingModel(x, exponent=1, "Thurstone's 1st data set") # voting model f(x)^1

# ╔═╡ 87c289db-7964-4660-b85a-fa34d874caa9
modelComparison1("Thurstone vs Voting Model f(x)^1", x=[0.36129, 0.316129, 0.322581], y=[0.47, 0.30, 0.23]) 

# ╔═╡ be84d6a6-8f5b-478d-bdcc-a2bb1359f02a
discreteVotingModel(x, exponent=2, "Thurstone's 1st data set") # hybrid voting model

# ╔═╡ bd91ba19-be82-4335-90b9-d3d533eb51e0
modelComparison1("Thurstone vs Hybrid Voting Model f(x)^2", x=[0.241199, 0.250936, 0.507865], y=[0.47, 0.30, 0.23])

# ╔═╡ d61e0a82-1f4f-4ef4-8865-9e45466e1b44
discreteVotingModel(x, exponent=3, "Thurstone's 1st data set") # hybrid voting model

# ╔═╡ 7dd9c664-a6a3-4867-9ff5-6b487b8f1207
modelComparison1("Thurstone vs Hybrid Voting Model f(x)^3", x=[0.150163, 0.160925, 0.688912], y=[0.47, 0.30, 0.23])

# ╔═╡ 1261f045-99d1-4580-a926-6bba736ec1fe
md"
Though we have a high positive correlation between the predictions of Thurstone's and our votin model they are not perfect. This seems due to the fact that Thurstone's model predictions are more discriminative between the stimuli than our voting model. Further modification of the voting toward a *hybrid* model by introducing of an $exponent > 1$ to shift the model towards Thurstone shows no improvement. 
A test for the cognitive validity of either model cannot be made here because Thurstone's 'data' are only artificial and not behavioral. This necessary empirical test has to wait until we study our own data set (Ahrens & Möbus, 1968).
"
...
discreteVotingModel(S1, "Civil Courage Ratings", k = 6, m=7)

# ╔═╡ 380be5b0-7d95-42f4-ac92-98d151b3851a
function modelComparison2(;x=[], y=[], xlabel="VotingModel", ylabel="Thurstone Model")
	let x = x                             # probs of voting model
		y = y                             # probs of Thurstone model (p.245)
		rxy = trunc(Statistics.cor(x, y), digits=3)
		xs = [0, 1]; ys = [0, 1]
		#---------------------------------------------------------------------------
		plot(xs, ys, xlims=(-0.01, 1.0), ylims=(-0.01, 1.0), title="Model Comparison", seriestype=:line, colour=:red, xlabel=xlabel, ylabel=ylabel, label="line of perfect model agreement")
		#---------------------------------------------------------------------------
...
function modelDataComparison(;x=[], y=[], xlabel="VotingModel", ylabel="Ratings Data")
	let x = x                             # probs of voting model
		y = y                             # probs of Thurstone model (p.245)
		rxy = trunc(Statistics.cor(x, y), digits=3)
		xs = [0, 1]; ys = [0, 1]
		#---------------------------------------------------------------------------
		plot(xs, ys, xlims=(-0.01, 1.0), ylims=(-0.01, 1.0), title="Data-Model-Comparison", seriestype=:line, colour=:red, xlabel=xlabel, ylabel=ylabel, label="line of perfect agreement")
		#---------------------------------------------------------------------------
		plot!((x[1], y[1]), seriestype=:scatter, colour=:aquamarine, label="S1")
		plot!((x[2], y[2]), seriestype=:scatter, colour=:red, label="S2")
		plot!((x[3], y[3]), seriestype=:scatter, colour=:green, label="S3")
...
modelComparison2(x=[0.151899, 0.243671, 0.196203, 0.196203, 0.120253,0.0917722], y=[.11, .10, .06, .31, .17, .25])

# ╔═╡ 9441e656-5fb4-45f2-b4e9-394a08110d6a
modelDataComparison(x=[0.151899, 0.243671, 0.196203, 0.196203, 0.120253,0.0917722], y=[.33, .28, .24, .10, .05, .00])
...
discreteVotingModel(S2, "Liberality Ratings", k = 6, m=7)

# ╔═╡ 54c8c91c-2397-433a-9d3f-cd337154bd0b
modelComparison2(x=[0.248705, 0.176166, 0.0984456, 0.0259067, 0.222798, 0.227979], y=[.11, .10, .06, .31, .17, .25])

# ╔═╡ ad87afd5-a63f-417e-854b-76526efc930f
modelDataComparison(x=[0.248705, 0.176166, 0.0984456, 0.0259067, 0.222798, 0.227979], y=[.33, .28, .24, .10, .05, .00])
...
plotRatingDistributions2(S3, "Sincerity Ratings")

# ╔═╡ b8c1072f-c206-4c0b-a088-f10c1491f80a
discreteVotingModel(S3, "Sincerity Ratings", k = 6, m=7)

# ╔═╡ 80f8adc0-1e60-4008-9963-36d4ead0f562
modelComparison2(x=[0.296296, 0.117284, 0.0617284, 0.0, 0.234568, 0.290123], y=[.11, .10, .06, .31, .17, .25])

# ╔═╡ 70c13302-72e0-4dbb-85c0-3cee2a9f0272
modelDataComparison(x=[0.296296, 0.117284, 0.0617284, 0.0, 0.234568, 0.290123], y=[.33, .28, .24, .10, .05, .00])
...
discreteVotingModel(S4, "Intelligence Ratings", k = 6, m=7)

# ╔═╡ 2c0d5a84-6dfa-4855-ba41-764cb3d7b078
modelComparison2(x=[0.286957, 0.252174, 0.0434783, 0.252174, 0.0, 0.165217], y=[.11, .10, .06, .31, .17, .25])

# ╔═╡ 4b05297c-aa2d-40e3-9e48-67317fcbdb44
modelDataComparison(x=[0.286957, 0.252174, 0.0434783, 0.252174, 0.0, 0.165217], y=[.33, .28, .24, .10, .05, .00])
...
discreteVotingModel(S5, "Objectivity Ratings", k = 6, m=7)

# ╔═╡ 8c21b6ed-9f4c-4e08-9d88-2c621a68394a
modelComparison2(x=[0.239819, 0.171946, 0.108597, 0.0452489, 0.19457, 0.239819], y=[.11, .10, .06, .31, .17, .25])

# ╔═╡ 980ca609-9885-44aa-99b8-035d697f2610
modelDataComparison(x=[0.239819, 0.171946, 0.108597, 0.0452489, 0.19457, 0.239819], y=[.33, .28, .24, .10, .05, .00])
