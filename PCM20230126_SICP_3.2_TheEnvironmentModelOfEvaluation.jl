...
# ╔═╡ 553af00d-cbdb-4342-bb6e-f544f98f07c1
using Plots
...
# ╔═╡ a00da065-0d69-4f7b-99e0-0ba90a4f13a7
range(0, 10, length=100)

# ╔═╡ 590decda-bbf7-4cbd-96ff-e64acfe9e8a0
typeof((0.4, 1.4))

# ╔═╡ 9e8517d9-d9d1-4902-92de-182ce0a7201c
rectangle(w, h, x, y) = Shape(x .+ [0,w,w,0], y .+ [0,0,h,h])

# ╔═╡ 74b6d080-6466-41d5-9a59-c6e4fec99fa3
square(w, x, y) = rectangle(w, w, x, y) 	# square(width, x-coord, y-coord) 

# ╔═╡ a5bc7049-e433-4fc6-b0a5-2b8e1c884ea5
begin
	#----------------------------------------------------------------
	plot(title="Structure of Environment Frames", xlims=(0, 14), ylims=(0,14), scale=:same, legend=:no, showaxis=:no, titlelocation = :center)
	plot!(square(4.0, 5.5, 9.0), fillcolor=plot_color(:cornflowerblue, 0.2)) # E_I
	annotate!( 8.5, 12.0, ("E_I", 14, :blue))   # E_I
	annotate!( 6.3, 10.6, ("x: 3", 12, :blue))  # E_I
	annotate!( 6.3,  9.6, ("y: 5", 12, :blue))  # E_I
	plot!(square(4.0, 2.0, 2.0), fillcolor=plot_color(:cornflowerblue, 0.2)) # E_II
	annotate!( 5.0,  5.0, ("E_II",  14, :blue)) # E_II
	annotate!( 2.8,  3.6, ("z: 6", 12, :blue))  # E_II
	annotate!( 2.8,  2.6, ("x: 7", 12, :blue))  # E_II
	plot!(square(4.0, 9.0, 2.0), fillcolor=plot_color(:cornflowerblue, 0.2)) # E_III
	annotate!(12.0,  5.0, ("E_III", 14, :blue)) # E_III
	annotate!( 9.8,  3.6, ("m: 1", 12, :blue))  # E_III
	annotate!( 9.8,  2.6, ("y: 2", 12, :blue))  # E_III
	annotate!( 4.0,  0.0, ("A", 14, :black))
	annotate!(11.0,  0.0, ("B", 14, :black))
...
md"
###### *Pointers* to environments are represented in *Julia* as *assignments*  $$\;\;=\;\;$$
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "153c6c831c032927c14e8383a2274b96fa83b229"
