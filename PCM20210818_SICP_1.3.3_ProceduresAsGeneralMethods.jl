...
# ╔═╡ c4bda8c7-a48e-4048-931d-3db177e2f621
begin
	using Pluto
	using Plots	
...
md"
=====================================================================================
#### SICP: [1.3.3 Procedures As General Methods](https://mitp-content-server.mit.edu/books/content/sectbyfn/books_pres_0/6515/sicp.zip/full-text/book/book-Z-H-12.html#%_sec_1.3.3)
...
# ╔═╡ 030035a8-8458-45e1-8c90-a9bbc78bd487
md"
---
##### 0. Introduction
Here we deal with methods for finding *zeros* and *fixed points* of functions.

"

# ╔═╡ a92d27bb-c4d3-45a8-a671-a1fde6225a36
md"
---
##### 1. Topics
...
# ╔═╡ 6fff313a-5b91-4190-b73a-97f599bc301f
md"
---
##### 2. Libraries
"
...
md"
---
##### 3.1 Finding Roots of Equations by the Half-interval Method
"
...
function halfIntervalMethod(f, a, b)
	...
	let (a_value, b_value) = (f(a), f(b))             # parallel assignment
		...
		elseif negative(b_value) && positive(a_value) # '&&' must be infix !
			...
			error("values are not of opposite sign, a=$a, b=$b")
		end # if
	end # let
...
# Julia's binary prefix '-' makes the expression more complicated as the multi-ary Scheme operator '-'
halfIntervalMethod(x-> -(-(^(x, 3), *(2, x)), 3), 1.0, 2.0) # prefix

# ╔═╡ 03cf7533-2056-4b8d-93ba-aabd18517b11
halfIntervalMethod(x-> x^3 - 2.0x - 3.0, 1.0, 2.0)     # infix and direct e.g. 2x

# ╔═╡ 01678534-e80a-47bd-b2b2-7c508f1504aa
md"
##### 3.2 Finding *Fixed Points* of Functions 
---
...
let fixPoint = round(fixedPoint(cos, 1.0), digits=4)
	...
	annotate!(1.35, -0.5, ("fixpoint=$fixPoint", 10))
end
...
let fixPoint = round(fixedPoint(y->sin(y)+cos(y), 1), digits=4)
	...
	annotate!(2.0, -0.5, ("fixpoint=$fixPoint", 10))
end
...
# ╔═╡ c2f42be7-8fd8-4f90-bfc2-a256c09828eb
md"
StackOverFlow expected: due to missing *tco* (tail-recursive optimization) in Julia
"

# ╔═╡ 94d1eb71-d09a-4f0b-b7e3-0700026ebe72
mySqrt(2.0) # StackOverFlow due to missing tco (tail-recursive optimization) in Julia
...
function fixedPointIter(f::Function, first_guess::FloatOrSigned)::FloatOrSigned
	#----------------------------------------------
	closeEnough(v1, v2) = abs(v1-v2) < tolerance
	#----------------------------------------------
	function myTry(guess)
	...
	myTry(first_guess)
	#-----------------------------------------------
end # function fixedPointIter

# ╔═╡ 50c5ed96-22ad-4a09-be21-9ea23946fb78
fixedPointIter(cos, 1.0)

# ╔═╡ 8ba154a1-f3f2-4427-a5dc-c357fb8ed079
fixedPointIter(y -> sin(y) + cos(y), 1.0)

# ╔═╡ 2f0df9c1-462d-49fe-b79f-58da37db1d4b
function mySqrtIter(x)
	#-------------------------------------
	average(x, y) = (x + y)/2.0
	#-------------------------------------
	fixedPointIter(y -> average(y, x/y), 1.0)
	#-------------------------------------
...
let (x, average) = (2.0, (x, y) -> (x + y)/2.0)
	...
	plot!([-1.0, +2.0], [-1.0, 2.0], line=(1, :cornflowerblue), label=L"x=f(x)")
	annotate!(2.5, 0.3, ("fixpoint=$fixPoint", 10))
end
...
let (x, average) = (9.0, (x, y) -> (x + y)/2.0)
	...
	annotate!(4.0, 1.0, ("fixpoint=$fixPoint", 10))
end
...
function fixedPointIter2(f::Function, first_guess::FloatOrSigned)::FloatOrSigned
	#----------------------------------------------
	closeEnough(v1, v2) = abs(v1-v2) < tolerance
	#----------------------------------------------
	guess = first_guess
	...
	next
end # function fixedPointIter2

# ╔═╡ 0d5abce7-12bc-47fa-9dd0-f54eaf1e86bb
fixedPointIter2(cos, 1.0)

# ╔═╡ 0ce1f847-08a6-43f5-8f0f-3e78678c2bb1
fixedPointIter2(y->sin(y)+cos(y), 1.0)

# ╔═╡ 75de942b-f4d9-444d-9ffc-339e25fdfe82
function mySqrtIter2(x)
	#-------------------------------------
	average(x, y) = (x + y)/2.0
	#-------------------------------------
	fixedPointIter2(y->average(y, x/y), 1.0)
	#-------------------------------------
end # function mySqrtIter2
...
let (x, average) = (2.0, (x, y) -> (x + y)/2.0)
	...
	plot!([-0.5, +2], [fixPoint, fixPoint], line=(2, :magenta), label=L"\{(.,y)|y=x/y\}")
	plot!([-1.0, 5.0], [-1.0, 5.0], line=(1, :cornflowerblue), label=L"x=f(x)")
	annotate!(3.5, 1.0, ("fixpoint=$fixPoint", 10))
end
...
let (x, average) = (9.0, (x, y) -> (x + y)/2.0)
	...
	plot!([-1.0, 5.0], [-1.0, 5.0], line=(1, :cornflowerblue), label=L"x=f(x)")
	annotate!(1.0, 2.0, ("fixpoint=$fixPoint", 10))
end
...
mySqrtIter2(mySqrtIter2(2.0) + mySqrtIter2(3.0))
...
mySqrtIter2(1000.0)^2

# ╔═╡ afd22730-1ae2-4274-98d3-3ed92617ac97
md"
---
##### 5. Summary
We defined the $√$-function both as a *root*- and a *fixpoint*-searching method.
...
manifest_format = "2.0"
project_hash = "c76d37e8417e19b8fa9121895f9707c9b4941234"
